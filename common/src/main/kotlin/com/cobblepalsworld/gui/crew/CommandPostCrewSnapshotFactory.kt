        ...
        val members = CommandPostCrewManager.findMembers(dimensionId, routerPos)
            .map { member ->
                ...
                CommandPostCrewMemberSnapshot(
                    ...
                    speciesIdentifier = pokemon?.species?.resourceIdentifier?.toString() ?: member.speciesIdentifier,
                    aspects = pokemon?.aspects ?: member.aspects,
                    heldItemId = pokemon?.heldItem()?.takeUnless { it.isEmpty }?.let { Registries.ITEM.getId(it.item).toString() } ?: member.heldItemId,
