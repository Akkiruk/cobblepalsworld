...
class PokemonTagScreenHandler : ScreenHandler {
    ...
    constructor(syncId: Int, playerInventory: PlayerInventory, pokemonId: UUID) : super(MenuTypes.POKEMON_TAG.get(), syncId) {
        ...
        if (existing != null) {
            ...
            for ((i, entry) in existing.augments.augments.withIndex()) {
    ...
    override fun quickMove(player: PlayerEntity, slotIndex: Int): ItemStack {
        ...
        if (stack.item is TagItem && slots[TAG_SLOT].stack.isEmpty) {
            ...
            // Try to insert into first empty augment slot
            for (i in AUGMENT_SLOT_START until AUGMENT_SLOT_END) {
                if (slots[i].stack.isEmpty) {
                    slots[i].stack = stack.split(1)
                    slot.markDirty()
                    break
                }
            }
