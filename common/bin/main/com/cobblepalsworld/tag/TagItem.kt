...
class TagItem(val tagType: TagType, settings: Settings) : Item(settings) {
    ...
    override fun useOnBlock(context: ItemUsageContext): ActionResult {
        ...
        if (!world.isClient) {
            if (world.getBlockEntity(pos) is Inventory) {
                ...
                player.sendMessage(
                    Text.literal("Not a container!").formatted(Formatting.RED),
                    true
                )
    ...
    override fun appendTooltip(
        ...
        if (tagType.supportsBinding) {
            ...
            if (bound != null) {
                tooltip.add(Text.literal("Bound: ${bound.x}, ${bound.y}, ${bound.z}").formatted(Formatting.GREEN))
            } else {
                tooltip.add(Text.literal("Sneak+Use on container to bind").formatted(Formatting.YELLOW))
            }
            tooltip.add(Text.literal("Sneak+Use on air to clear").formatted(Formatting.DARK_GRAY))
        }
    }
}
